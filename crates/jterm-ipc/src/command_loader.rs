...
/// Get the default commands directory (`~/.config/jterm/commands/`).
pub fn default_commands_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".config").join("jterm").join("commands"))
}
...
"#,
    ...
    fn test_default_commands_dir() {
        ...
        assert!(dir.to_string_lossy().contains(".config/jterm"));
    }
}
