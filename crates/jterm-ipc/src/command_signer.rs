...
//!
//! Official jterm commands are signed with a known public key embedded in the
...
pub enum VerifyResult {
    /// Signed with the official jterm key.
    OfficialVerified,
...
pub fn verify_command(toml_content: &str, signature_hex: Option<&str>) -> VerifyResult {
    ...
    if let Ok(official_key) = VerifyingKey::from_bytes(&JTERM_PUBLIC_KEY) {
...
"#;
        ...
        // Verify with the matching key (uses third-party path since it won't match JTERM_PUBLIC_KEY)
        ...
        // (because the keypair doesn't match the placeholder JTERM_PUBLIC_KEY)
