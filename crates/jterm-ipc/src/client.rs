...
//!
//! Connects to the jtermd daemon socket and sends [`IpcRequest`] messages,
...
pub enum ClientError {
    ...
    #[error("connection failed: is jtermd running? (socket: {0})")]
...
/// An IPC client that connects to the jtermd daemon.
...
impl IpcClient {
    ...
    /// Create a client using the default socket path from jterm-session.
    pub fn default_path() -> Self {
        Self::new(jterm_session::daemon::socket_path())
    }
