...
//!
//! Commands are external scripts that communicate with jterm via
...
//! that jterm sends back (via the command's stdin).

use serde::{Deserialize, Serialize};

/// Message sent FROM a command script TO jterm (via the script's stdout).
///
/// Each variant maps to a UI interaction that jterm presents to the user.
/// The command writes one JSON line per message and then waits for jterm
...
/// Response sent FROM jterm TO the command script (via the script's stdin).
///
/// jterm writes one JSON line per response after the user completes the
